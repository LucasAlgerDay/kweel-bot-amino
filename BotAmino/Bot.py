...
class Bot(SubClient, ACM):
    def __init__(self, client: Client, community, prefix: str = "!", bio: str = None, activity: bool = False) -> None:
        ...
        z=test2.count_documents({"_id":self.community_amino_id})
        if z==0:
        	self.create_community_file()
        
        z=test4.count_documents({"_id":self.community_amino_id})
        if z==0:
    ...
    def create_community_filee(self):
    	it=self.create_dic()
    	#dict = self.create_dict()
    	#test2.insert_one(dict)
    	test4.insert_one(it)
    ...
    def create_dict(self):
        return {"_id":self.community_amino_id,"welcome": "", "prefix": self.prefix, "welcome_chat": [], "locked_command": [], "favorite_users": [], "favorite_chats": [], "banned_words": []}

    def create_dic(self):
        return {"_id":self.community_amino_id,"muted_users": [], "comids": []}
    def get_dict(self):
    ...
    def get_wel(self):
    	...
    	welcm=wf.read()
    	return welcm
    def get_file_inf(self, info: str = None):
        res=test4.find({"_id":self.community_amino_id})
        for ress in res:
        	reh=ress[info]
        return reh
    ...
    def set_prefix(self, prefix: str):
        self.prefix = prefix
        filter = { '_id': self.community_amino_id}
        newvalues = { "$set": { 'prefix': prefix }}
        test2.update_one(filter, newvalues)

    def set_welcome_message(self, message: str):
        self.message_bvn = message
        filter = { '_id': self.community_amino_id}
        newvalues = { "$set": { 'welcome': message}}
        test2.update_one(filter, newvalues)
        #self.sewel(message)

    def set_welcome_chat(self, chatId: str):
        test2.update_one({ "_id": self.community_amino_id },{ "$push": { "welcome_chat": chatId }})

    def add_favorite_users(self, value: str):
    	test2.update_one({ "_id": self.community_amino_id },{ "$push": { "favorite_users": value }})

    def add_muted_users(self, value: str):
    	test4.update_one({ "_id": self.community_amino_id },{ "$push": { "muted_users": value }})
    def add_favorite_chats(self, value: str):
    	test2.update_one({ "_id": self.community_amino_id }, { "$push": { "favorite_chats": value }})

    def add_banned_words(self, liste:list):
    	for value in liste:
    		test2.update_one({ "_id": self.community_amino_id }, { "$push": { "banned_words": value }})

    def remove_muted_users(self, value: str):
    	test4.update_one({ "_id": self.community_amino_id },{ "$pull": { "muted_users": value }})
    def add_comids(self, value: int):
    	test4.update_one({ "_id": self.community_amino_id },{ "$push": { "comids": value }})
    def remove_comids(self, value: int):
    	test4.update_one({ "_id": self.community_amino_id },{ "$pull": { "comids": value }})
    def add_locked_command(self, liste:list):
        for value in liste:
        	test2.update_one({ "_id": self.community_amino_id }, { "$push": { "locked_command": value }})

    def remove_favorite_users(self, value: str):
        test2.update_one({ "_id": self.community_amino_id }, { "$pull": { "favorite_users": value }})

    def remove_favorite_chats(self, value: str):
        test2.update_one({ "_id": self.community_amino_id }, { "$pull": { "favorite_chats": value }})

    def remove_banned_words(self, liste: list):
        for value in liste:
        	test2.update_one({ "_id": self.community_amino_id }, { "$pull": { "banned_words": value }})

    def remove_locked_command(self, liste:list):
        for value in liste:
        	test2.update_one({ "_id": self.community_amino_id }, { "$pull": { "locked_command": value }})

    def unset_welcome_chat(self,chatId:str):
        test2.update_one({ "_id": self.community_amino_id }, { "$pull": { "welcome_chat": chatId }})
    ...
    def check_new_member(self):
        if not (self.message_bvn or self.welcome_chat):
            return
        new_list = self.get_all_users(start=0, size=20, type="recent")
        ...
        new_users = self.get_all_users(start=0, size=25, type="recent")
        self.new_users = [elem["uid"] for elem in new_users.json["userProfileList"]]
