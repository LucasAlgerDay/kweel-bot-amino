...
import ast
import random
#sels=(random.randint(2,9))
#f=open(f"proxies/{sels}.txt")
#dat = f.read()
#proxs= ast.literal_eval(dat)
#prox={'sock5':'37.221.193.221:15012','socks5':'37.221.193.221:15012'}
#prox={'http': 'socks5://vedu:4321@premium.zaicadiahost.xyz:7875','https':'socks5://vedu:4321@premium.zaicadiahost.xyz:7875'}
#prox={'https':'socks5://51.254.69.243:3128','https':'socks5://51.254.69.243:3128'}
#prox={"http":"http://fzaynofi:xl0q9jnm9trx@188.74.210.21:6100","https":"http://fzaynofi:xl0q9jnm9trx@188.74.210.21:6100"}
#prox={'http':'8.219.97.248:80','https':'8.219.97.248:80'}
#prox={'https':'socks5://47.91.88.100:1080','https':'socks5://47.91.88.100:1080'}
#prox={"http":"http://fzaynofi@185.199.229.156:7492","https":"http://fzaynofi@185.199.229.156:7492"}
#@dorthegra/IDörthe#8835 thanks for support!
#prox={'http':'socks5://37.221.193.221:15012','https':'socks5://37.221.193.221:15012'}
class Client(Callbacks, SocketHandler):
    def __init__(self, deviceId="19E9BC622C09DE7D042BD43DAA1140853BAB811715031F811D7321826D012D6BC99BCCB51E8DCCEF8F", userAgent: str="Apple iPhone13 iOS v16.1.2 Main/3.13.1", proxies= {'http': 'socks5://itskwel999:zNFqCpVPeB@185.228.194.156:50101','https': 'socks5://itskwel999:zNFqCpVPeB@185.228.194.156:50101'}, certificatePath = None, socket_trace = False, socketDebugging = False, socket_enabled = True, autoDevice = False, sub: bool = False):
    ...
    def join_voice_chat(self, comId: str, chatId: str, joinType: int = 1, keep_alive: bool = True):
        ...
        **Parameters**
        ...
        if keep_alive and chatId not in self.active_live_chats:
            ...
            self.send(data)
    def jon_voice_chat(self, comId: str, chatId: str, joinType: int = 1):
        """
        Joins a Voice Chat
    ...
    def leave_aud(self, comId: str, chatId: str):
    ...
    def join_screen_room(self, comId: str, chatId: str, keep_alive: bool = True):
        data = {
            "o":
                {
                    "ndcId": int(comId),
        ...
        if keep_alive and chatId not in self.active_live_chats:
            ...
            self.send(data)
    def jocreen_room(self, comId: str, chatId: str, joinType: int=2):
        data = {
            "o":
                {
                    "ndcId": int(comId),
    ...
    def play_video(self, comId: str, chatId: str):
        ...
        true = True
        false = False
        data={"o": {"ndcId": int(comId), "threadId": chatId, "joinRole": 1, "id": "10335106"}, "t": 112}
        data = json.dumps(data)
        time.sleep(1)
        self.send(data)
        data={"o": {"ndcId": int(comId), "threadId": chatId, "channelType": 5, "id": "10335436"}, "t": 108}
        data = json.dumps(data)
        time.sleep(1)
        self.send(data)
        data={"o": {"ndcId": int(comId), "threadId": chatId, "playlist": {"currentItemIndex": 0, "currentItemStatus": 1, "items": [{"author": null, "duration": 28.815, "isDone": false, "mediaList": [[100, ".tva.png", null]], "title": "Birthday 🥳", "type": 1, "url": "file://choti.mp3"}]}, "id": "10336041"}, "t": 120}
        data = json.dumps(data)
        time.sleep(1)
        self.send(data)
        data={"o": {"ndcId": int(comId), "threadId": chatId, "playlist": {"currentItemIndex": 0, "currentItemStatus": 2, "items": [{"author": null, "duration": 28.815, "isDone": false, "mediaList": [[100, "http://pm1.narvii.com/8083/3bec7c4a86180dd77e10763ae021cd0d4da4735ar1-512-410v2_00.jpg", null]], "title": "Love forever", "type": 1, "url": "file://https://youtu.be/NpJ6lnbgi_s"}]}, "id": "10337809"}, "t": 120}
        data = json.dumps(data)
        time.sleep(2)
        self.send(data)
        data={"o": {"ndcId": int(comId), "threadId": chatId, "playlist": {"currentItemIndex": 0, "currentItemStatus": 2, "items": [{"author": "Ravi", "duration": 76000.815, "isDone": true, "mediaList": [[100, "http://mm1.narvii.com/8297/5618150ea391fb13d90cf0e25777152001ce7d2a1a58fa99r7-300-168_00.jpg", null]], "title": "Alexa ki Shaadi", "type": 1, "url": "file://tv.mp4"}]}, "id": "10366159"}, "t": 120}
        data = json.dumps(data)
        time.sleep(2)
        self.send(data)
        self.active = True
        threading.Thread(target=self.run_vc(comId, chatId, joinType)).start()
    ...
    def run_vc(self, comId: str, chatId: str, joinType: str):
        while chatId in self.active_live_chats:
            ...
            self.send(data)
            sleep(60)
    def rn_vc(self, comId: str, chatId: str, joinType: str):
        while self.active:
            data = {
                "o": {
                    "ndcId": comId,
    ...
    def start_vido_call(self, comId: str, chatId: str, joinType: int = 1):
        data = {
            "o": {
                "ndcId": comId,
                "threadId": chatId,
    ...
    def login(self, email: str, password: str):
        ...
        **Returns**
        ...
        response = self.session.post(f"{self.api}/g/s/auth/login", headers=self.parse_headers(data=data), data=data, proxies=self.proxies, verify=True,timeout=30)
    ...
    def lohgin(self, email: str, password: str):
        """
        Login into an account.
    ...
    def longin(self, email: str, password: str,proxy:bool=False):
        """
        Login into an account.
        ...
        **Returns**
        ...
        if proxy is True:
            data=json.dumps({"email":email,"password":password,"device":self.device_id})
            response=self.session.post("http://154.12.227.82:2193/login",data=data)
        ...
        if proxy is False:
            ...
            response = self.session.post(f"{self.api}/g/s/auth/login", headers=self.parse_headers(data=data,deviceId=self.devicee()), data=data, proxies=prox, verify=self.certificatePath)
    ...
    def register(self, nickname: str, email: str, password: str, verificationCode: str, deviceId: str = None):
        ...
        **Returns**
        ...
        response = self.session.post(f"{self.api}/g/s/auth/register", data=data, headers=headd)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
    ...
    def verify(self, email: str, code: str,dev:str,verifyInfoKey:bool=False,key=None):
        ...
        **Returns**
        ...
        response = self.session.post(f"{self.api}/g/s/auth/verify-account", headers=self.parse_headers(data=data), data=data, proxies=self.proxies, verify=self.certificatePath)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def change_pass(self,password,new_password):
        ...
        response = self.session.post(f"{self.api}/g/s/auth/verify-password", headers=self.parse_headers(data=data),data=data)
        if response.json()['api:statuscode']==0:
            ...
            res = self.session.post(f"{self.api}/g/s/auth/change-password", headers=self.parse_headers(data=dat),data=dat)
            return res.json()
        else:
    ...
    def request_verify_code(self, dev: str,email: str, resetPassword: bool = False,verifyInfoKey:bool =False,key=None):
        ...
        **Returns**
        ...
        headd["NDCAUTH"]=None
        response = self.session.post(f"{self.api}/g/s/auth/request-security-validation", headers=headd, data=data, proxies=self.proxies, verify=self.certificatePath)
    ...
    def get_user_info(self, userId: str):
        ...
        **Returns**
            ...
            - **Fail** : :meth:`Exceptions <amino.lib.util.exceptions>`
        """
        response = self.session.get(f"{self.api}/g/s/user-profile/{userId}", headers=self.parse_headers(), proxies=self.proxies, verify=self.certificatePath,timeout=30)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
    ...
    def random_community(self,search:str,start: int = 0, size: int = 25):
        response=self.session.get(f"{self.api}/g/s/community/search?q={search}&language=en&completeKeyword=1&start={start}&size={size}",headers=self.parse_headers(), proxies=self.proxies, verify=self.certificatePath)
    ...
    def get_community_info(self, comId: str):
        ...
        **Returns**
            ...
            - **Fail** : :meth:`Exceptions <amino.lib.util.exceptions>`
        """
        response = self.session.get(f"{self.api}/g/s-x{comId}/community/info?withInfluencerList=1&withTopicList=true&influencerListOrderStrategy=fansCount", headers=self.parse_headers(), proxies=self.proxies, verify=self.certificatePath,timeout=30)
    ...
    def devicee(self):
        d=self.session.get("https://ka-generator.herokuapp.com/device").text
        return d
