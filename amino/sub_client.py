...
class SubClient(client.Client):
    ...
    def restarts(self):
    	key="6da3ac15-23f8-41c9-ac10-35b6b0641c69"
    	app_name="alexa17171"
    ...
    def delete_bubble(self, bubbleId: str):
        """
        Block an User.
        ...
        **Returns**
        ...
        """
        response = self.session.delete(f"{self.api}/x{self.comId}/s/chat/chat-bubble/{bubbleId}", headers=self.parse_headers(), proxies=self.proxies, verify=self.certificatePath)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def edit_profile(self, nickname: str = None, content: str = None, icon: BinaryIO = None, chatRequestPrivilege: str = None, imageList: list = None, captionList: list = None, backgroundImage: str = None, backgroundColor: str = None, titles: list = None, colors: list = None, defaultBubbleId: str = None,fileType: str = None):
        mediaList = []
        ...
        if captionList is not None:
            for image, caption in zip(imageList, captionList):
                mediaList.append([100, self.upload_media(image, fileType), caption])

        else:
            if imageList is not None:
                for image in imageList:
                    mediaList.append([100, self.upload_media(image, fileType), None])
    ...
    def full_embed(self, link, image, message, chatId):
        ...
        data = json.dumps(data)
        response = self.session.post(f"{self.api}/x{self.comId}/s/chat/thread/{chatId}/message", headers=headers.ApisHeaders().headers, data=data, proxies=self.proxies, verify=self.certificatePath)
    ...
    def get_chat_bubbles(self, start: int = 0, size: int = 25):
        response = self.session.get(f"{self.api}/x{self.comId}/s/chat/chat-bubble?type=all-my-bubbles&start={start}&size={size}", headers=self.parse_headers(), proxies=self.proxies, verify=self.certificatePath)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else:
    ...
    def send_active_obj(self, startTime: int = None, endTime: int = None, optInAdsFlags: int = 2147483647, tz: int = -timezone // 1000, timers: list = None, timestamp: int = int(timestamp() * 1000)):
        ...
        response = self.session.post(f"{self.api}/x{self.comId}/s/community/stats/user-active-time", headers=headers.ApisHeaders(data=data, deviceId=device).headers, data=data, proxies=self.proxies, verify=self.certificatePath)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def get_chat_lurkers(self, start: int = 0, size: int = 100):
        response = self.session.get(f"{self.api}/x{self.comId}/s/live-layer/public-chats?start={start}&size={size}", headers=self.parse_headers(), proxies=self.proxies, verify=self.certificatePath)
